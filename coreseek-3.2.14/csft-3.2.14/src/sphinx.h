...
#define CORESEEK_VERSION		"3.2 [ Sphinx " SPHINX_VERSION "]"
...
public:
	...
	/// This used by highlight, the max weight is 100, NOT used in 0.9.9s
	virtual int						GetWordWeight (BYTE * , int ) const { return 100; }
...
protected:
	...
	int								m_iLastTokenBufferLen;		///< the buffer length -- coreseek;	
...
struct CSphColumnInfo
{
	...
	int				m_iMVAIndex;	///< index the mva indexer, for faster reassign values.
...
public:
	...
	virtual	void			BuildHits ( BYTE ** dFields, int iFieldIndex, int iStartPos );
