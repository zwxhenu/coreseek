...
bool CSphConfigParser::IsPlainSection ( const char * sKey )
{
	...
	if ( !strcasecmp ( sKey, "python" ) )		return true;
	return false;
}
...
bool CSphConfigParser::ValidateKey ( const char * sKey )
{
	...
	else if ( m_sSectionType=="python" )	pDesc = g_dKeysPython;
	if ( !pDesc )
	{
	...
	if (!bNoCheck) {
		if (m_sSectionType == "analyzer" || m_sSectionType == "query")
			bNoCheck = true;
	}
...
bool sphConfTokenizer ( const CSphConfigSection & hIndex, CSphTokenizerSettings & tSettings, CSphString & sError )
{
	...
	{
		tSettings.m_sDictPath = hIndex["charset_dictpath"];
		tSettings.m_iType = TOKENIZER_ZHCN_GBK;
	}
	else if ( hIndex("charset_dictpath") &&hIndex["charset_type"]=="zh_tw.big5")
	{
		tSettings.m_sDictPath = hIndex["charset_dictpath"];
		tSettings.m_iType = TOKENIZER_ZHTW_BIG5;
	} 
 #endif
#endif
