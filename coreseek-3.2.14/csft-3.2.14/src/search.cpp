...
int main ( int argc, char ** argv )
{
	...
	while ( hConf["index"].IterateNext () )
	{
		...
		const char * sQueryInfoPre = NULL;	//for set names
		...
		while ( !bNoInfo )
		{
			...
			if (hSource("sql_query_info_pre"))
			{
				sQueryInfoPre = hSource["sql_query_info_pre"].cstr();
			}
		...
		if ( pResult->m_dMatches.GetLength() )
		{
			...
			for ( int i=iStart; i<iMaxIndex; i++ )
			{
				...
				if ( sQueryInfo )
				{
					...
					if (sQueryInfoPre)
					{
						if(mysql_query ( &tSqlDriver, sQueryInfoPre ))
