...
namespace CRFPP {
  ...
  bool EncoderFeatureIndex::export_mmap(const char *filename)
  {
	  std::vector <char *> key;
	  std::vector <int>    val;
	  for (std::map<std::string, std::pair<int, unsigned int> >::iterator
	  ...
	  bofs.write(reinterpret_cast<char *>(&fsize), sizeof(fsize)); 
	  bofs.write(reinterpret_cast<char *>(&dsize), sizeof(dsize)); //da-size
...
#define ADD_COST(T, A) \
    ...
    if (alpha_float_) ADD_COST(float,  alpha_float_)
		...
		double c = 0; 
		for (int *f = n->fvector; *f != -1; ++f) c += (alpha_)[*f + n->y]; 
			n->cost =cost_factor_ *(double)c;
	}
#undef ADD_COST
  }
...
#define ADD_COST(T, A) \
    ...
    if (alpha_float_) ADD_COST(float,  alpha_float_)
		...
		double c = 0.0; 
		for (int *f = p->fvector; *f != -1; ++f) 
			c += (alpha_)[*f + p->lnode->y * y_.size() + p->rnode->y]; 
		p->cost =cost_factor_*(double)c;
	}
  }
#undef ADD_COST
}
